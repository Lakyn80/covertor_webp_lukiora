...
try:
    ...
    # HEIF plugin není – i tak poběží pro JPG/PNG
    HEIF_OK = False

# iPhone fotky: .heic/.heif (plus standardní)
...
def convert_one(
    ...
    """
    Return tuple(status, path). status ∈ {"ok", "skipped_exists", "err"}.
    """
    try:
        ...
        with Image.open(job.src) as im:
            # 1) Respektuj EXIF rotaci (iPhone ji používá)
            im = ImageOps.exif_transpose(im)

            # 2) Volitelné zmenšení (poměr zachován)
            if max_w or max_h:
                im.thumbnail(
            ...
            # 3) Sjednocení barev/kanálů
            # - 16bit/grayscale → RGB
            if im.mode in ("I;16", "I", "F"):
                im = im.convert("RGB")
            # - Paleta / LA → RGBA (kvůli alfě)
            ...
            if exif_bytes:
                # Pillow umí přibalit EXIF i do WebP (novější verze)
        ...
        # Zapiš detail chyby vedle zdrojového souboru
        err_file = job.src.with_suffix(f".ERROR.txt")
        try:
            err_file.write_text(
                f"Source: {job.src}\nTarget: {job.dst}\n\n{repr(e)}\n",
                encoding="utf-8",
...
def main():
    ...
    if not HEIF_OK:
        # Jemné varování, jen pokud user cíleně chce HEIC
    ...
    # Mazání originálů, pokud si to uživatel přeje
